...
Beregner 1X2-sannsynligheter, over/under og BTTS.
"""
...
AVG_GOALS = 1.35  # snitt mål per lag per VM-kamp (historisk)
...
def predict_match(home_attack: float, home_defense: float,
    ...
    """Returnerer sannsynlighetsmatrise og nøkkeltall for en kamp."""
...
def implied_probability(odds: float) -> float:
    """Konverter desimalodds til implisitt sannsynlighet."""
    return 1.0 / odds if odds > 0 else 0.0


def remove_margin(home_odds: float, draw_odds: float, away_odds: float) -> tuple[float, float, float]:
    """Fjern bookmaker-margin (overround) for å få rene sannsynligheter."""
